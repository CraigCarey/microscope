...
Meteor.methods({
    ...
    upvote: function(postId) {
        ...
        //}
        //
        //// user hasn't already upvoted this post
        //if (_.include(post.upvoters, this.userId)) {
        //    throw new Meteor.Error('invalid', 'Already upvoted this post');
        //}
        //
        //Posts.update(post._id, {
        //    // mongo's $addToSet adds an item to an array property if it doesn't already exist
        //    $addToSet: {upvoters: this.userId},
        //    // mongo's $inc increments an integer field
        //    $inc: {votes: 1}
        //});

        // the above did this in three steps (get, check, update), calling the db twice
        // this risks race conditions, better to do it all in one go
        var affected = Posts.update({
            // get all of the posts with this id, that hasn't (ne) been upvoted by this poster
            _id: postId,
            upvoters: {$ne: this.userId}
        }, {
            // ... and add this poster to the upvoters array
            $addToSet: {upvoters: this.userId},
            $inc: {votes: 1}
        });
